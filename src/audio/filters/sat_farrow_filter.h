#ifndef sat_farrow_filter_included
#define sat_farrow_filter_included
//----------------------------------------------------------------------

// TODO !!!

// the ratio between output and input sample rate.
...
//------------------------------
  ...
  // Index of last input sample that was read. First valid sample starts at 0
  int sampleIndexInput = -1;

  // Position of next output sample in input stream
  T sampleIndexOutput = 0.0;
...
//------------------------------
  ...
  void init() {
    // clear delay line
    for (int ix = 0; ix < NTAPS; ++ix) delayLine[ix] = 0;
    sampleIndexInput = -1;
    sampleIndexOutput = 0.0;
  }
  ...
  void process() {
    ...
    while (1) {
      ...
      T sio_fracInTap = sampleIndexOutput - (T)sio_int;
      SAT_Assert( (sio_fracInTap >= 0) && (sio_fracInTap < 1) );

      // Further split the fractional part into
      // - bank index
      // - fractional part within the bank

      int sio_intBank = floor(sio_fracInTap * (T) NBANKS);
      T sio_fracInBank = sio_fracInTap * (T) NBANKS - (T)sio_intBank;
      SAT_Assert( (sio_fracInBank >= 0) && (sio_fracInBank < 1) );
      ...
      while (sampleIndexInput < sio_int) {
        ++sampleIndexInput;
        for (int ix = NTAPS-1; ix > 0; --ix) delayLine[ix] = delayLine[ix-1];
        //----------
      ...
      for (int ixTap = 0; ixTap < NTAPS; ++ixTap) {
        ...
        T hornerSum = c(ixTap, sio_intBank, ORDER);
        int ixPower;
...
//------------------------------
  ...
  //static
  T c(int ixTap, int ixBank, int ixPower) {
