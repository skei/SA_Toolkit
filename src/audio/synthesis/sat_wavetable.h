#ifndef sat_table_waveform_included
#define sat_table_waveform_included
//----------------------------------------------------------------------
...
#define SAT_WAVETABLE_MAX_TABLES 32
...
//------------------------------
  ...
  int32_t             MNumTables;
  SAT_WaveTableTable  MTables[SAT_WAVETABLE_MAX_TABLES];
...
//------------------------------
  ...
  SAT_WaveTable() {
    MNumTables = 0;
    for (int32_t i=0; i<SAT_WAVETABLE_MAX_TABLES; i++) {
      MTables[i].topFreq = 0;
      MTables[i].len = 0;
  ...
  void fft(int32_t N, T *ar, T *ai) {
    ...
    */
    ...
    for (L = 1; L <= M; L++) {            // stage L
      ...
      Ui = 0.;
      Wr = cos(M_PI/(T)LE1);
      Wi = -sin(M_PI/(T)LE1); // Cooley, Lewis, and Welch have "+" here
      for (j = 1; j <= LE1; j++) {
...
//------------------------------
  ...
  */
  ...
  int32_t addTable(int32_t len, T *tableIn, T topFreq) {
    if (MNumTables < SAT_WAVETABLE_MAX_TABLES) {
      T* ttt = MTables[MNumTables].table = new T[len];
      MTables[MNumTables].len = len;
      MTables[MNumTables].topFreq = topFreq;
      ++MNumTables;
      for (long idx = 0; idx < len; idx++) ttt[idx] = tableIn[idx];
      return 0;
    }
    return MNumTables;
  }
  ...
  */
  ...
  T makeTable(int32_t len, T *ar, T *ai, T scale, T topFreq) {
    fft(len, ar, ai);
    if (scale == 0.0) {
      // calc normal
      T max = 0;
  ...
  */
  ...
  void fillTables(T *freqRe, T *freqIm, int32_t numSamples) {
    int32_t idx;
    ...
    const T minVal = 0.000001; // -120 dB
    ...
    T topFreq = 2.0 / 3.0 / maxHarmonic;
    // for subsquent tables, float topFreq and remove upper half of harmonics
    T *ar = new T [numSamples];
    T *ai = new T [numSamples];
    T scale = 0.0;
    while (maxHarmonic) {
...
//------------------------------
  ...
  void makeSaw(int32_t tableLen=2048) {
    ...
    int32_t idx;
    T *freqRe = new T [tableLen];
    T *freqIm = new T [tableLen];
    // make a sawtooth
  ...
  void makeWave(T *waveSamples, int32_t tableLen) {
    T *freqRe = new T [tableLen];
    T *freqIm = new T [tableLen];
    // take FFT
...
//------------------------------
  ...
  */
  ...
  T process(T t, T dt=0) {
    // grab the appropriate wavetable
    int32_t tableIdx = 0;
    while ((dt >= MTables[tableIdx].topFreq) && (tableIdx < (MNumTables - 1))) { ++tableIdx; }
    SAT_WaveTable* wavetable = &MTables[tableIdx];

    #ifdef SAT_WAVETABLE_LINEAR_INTERPOLATION

      T temp = t * wavetable->len;
      int32_t intPart = temp;
      T fracPart = temp - intPart;
      T samp0 = wavetable->table[intPart];
      if (++intPart >= wavetable->len) intPart = 0;
      T samp1 = wavetable->table[intPart];
...
//#undef SAT_WAVETABLE_LINEAR_INTERPOLATION
#undef SAT_WAVETABLE_MAX_TABLES
