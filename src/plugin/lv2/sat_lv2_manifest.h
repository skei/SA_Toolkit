...
//------------------------------
  ...
  void create_manifest_ttl() {
    ...
    sprintf(temp,"<urn:%s>\n",clap_descriptor->id);        //"urn:skei.audio/sat_debug";
...
//----------------------------------------------------------------------
  ...
  clap_descriptor = SAT_GLOBAL.REGISTRY.getDescriptor(i);
  clap_plugin = SAT_CreatePlugin(i,clap_descriptor,clap_host);
  ...
  for (uint32_t i=0; i<num_plugins; i++) {
    ...
    SAT_Lv2CreateManifest(clap_plugin,manifest_ttl,plugin_ttl);
