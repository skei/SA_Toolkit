  ...
  void handleQuantizedEvents() {
    ...
    if (events.read(&event)) {
      ...
      do {
        ...
        while (next_event < (current_time + SAT_AUDIO_SLICE_SIZE)) {
          handleEvent(event);
          if (events.read(&event)) next_event = event.time;
          else next_event = buffer_length; // ???
        }
        // process next slice
        if (remaining < SAT_AUDIO_SLICE_SIZE) {
          ...
          current_time += SAT_AUDIO_SLICE_SIZE;
          remaining -= SAT_AUDIO_SLICE_SIZE;
        }
      } while (remaining > 0);
    }
    else {
      // no events..
      do {
        if (remaining < SAT_AUDIO_SLICE_SIZE) state = voice.process(state,current_time,remaining);
