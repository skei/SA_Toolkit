...
class SAT_GraphWire : public SAT_ListNode {
...
class SAT_GraphModule : public SAT_ListNode {
...
//  }
  ...
  bool inside(double x, double y) {
    ...
    if ( (x >= xpos) && (x <  xpos + (MODULE_WIDTH  * S))
      && (y >= ypos) && (y <  ypos + (MODULE_HEIGHT * S))) return true;
    return false;
  }

  bool insideBase(double x, double y) {
    //double S = getWindowScale();
    if (y < (ypos + (PIN_YDIST * S) + (BASE_HEIGHT * S))) return true;
    return false;
  }

  int32_t insideInput(double x, double y) {
    //double S = getWindowScale();
    if (y < (ypos + (PIN_YDIST * S))) {
      int32_t pin = (x - xpos) / (PIN_XDIST * S);
      if (pin < numInputs) return pin;
    }
    return -1;
  }

  int32_t insideOutput(double x, double y) {
    //double S = getWindowScale();
    int32_t pin = (x - xpos) / (PIN_XDIST * S);
...
//----------------------------------------
  ...
  void drawModule(SAT_GraphModule* AModule) {
    ...
    double S = getWindowScale();
    double xdist  = PIN_XDIST * S;
    double ydist  = PIN_YDIST * S;
    double pinw   = PIN_WIDTH * S;
    double pinh   = PIN_HEIGHT * S;
  ...
  void drawWire(SAT_GraphWire* AWire) {
    ...
    x1 += (pinw * 0.5);
    y1 += (pinh * 0.5) + (MODULE_HEIGHT * S) - ydist;
    x2 += (pinw * 0.5);
    y2 += (pinh * 0.5);
...
//------------------------------
  ...
  void on_widget_mouseClick(double AXpos, double AYpos, uint32_t AButton, uint32_t AState, uint32_t ATime) override {
      ...
      if (changed) do_widget_redraw(this,0,0);
