#ifndef sat_downsample_filter_included
#define sat_downsample_filter_included
//----------------------------------------------------------------------
...
template <typename T>
class SAT_DownSampleFilter {

//------------------------------
private:
//------------------------------

  const T SAT_DownSample_Coefficients[32] = {
...
//------------------------------
  ...
  T process(T* buffer, uint32_t FACTOR) {
    T out;
    T s = buffer[0];
    MInput[MIndex] = s;
    s *= SAT_DownSample_Coefficients[0];
    int Z,z;
    for (Z=1,z=MIndex-1; Z<FIR_SIZE; --z,++Z) {
      s += SAT_DownSample_Coefficients[Z] * MInput[z&FIR_MASK];
    }
