#ifndef sat_envelope_included
#define sat_envelope_included
//----------------------------------------------------------------------
...
#define sat_env_rate_scale     10.0f // 30.0f
#define sat_env_threshold      0.001 //SAT_TINY
...
class SAT_ExpEnvelope {
  ...
  private:
    ...
    T                     MValue = 0;
    int32_t               MStage = 0;
    SAT_ExpEnvelopeStage<T>  MStages[5]; // -,a,d,s,r
  ...
  public:
    ...
    T calcRate(T ms) {
      ...
      T a = ms * sat_env_rate_scale; // 0..1 -> 0..25
      a = (a*a*a);
    ...
    T process(void) {
      ...
      if (fabs(target - MValue) <= sat_env_threshold) {
        MStage += 1;
      }
      return MValue;
    }
